...
INVISIBLE_END_TAG = "\u2063__END__"  # 사용자에겐 안 보이는 종료 마커
...
class ActionSchoolLocation(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        dispatcher.utter_message(
            text=f"학교 주소는 다음과 같습니다: {address}\u2063__END__"
        )
        return []
...
class ActionSchoolContact(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        dispatcher.utter_message(
            text=f"학교 대표 전화번호는 {phone} 입니다.\u2063__END__"
        )
        return []
...
class ActionSchoolTransport(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if not data:
            dispatcher.utter_message(
                text=f"셔틀버스 정보를 불러오는 데 실패했습니다.\u2063__END__"
            )
        ...
        full_message = "\n\n".join(messages)
        dispatcher.utter_message(
            text=f"셔틀버스 운행 정보입니다:\n\n{full_message}\u2063__END__"
        )
