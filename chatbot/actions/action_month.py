...
class ActionAcademicCalendar(BaseAction):
    ...
    def run(
        ...
        if not user_input:
            return self.fail(dispatcher, "확인할 학사 일정을 입력해주세요. 예: '3월', '2025년'")
        ...
        if year_match:
            ...
            dispatcher.utter_message(text=f"📅 {year}년 학사 일정입니다:\n" + "\n".join(result))
